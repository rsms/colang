...
Editor::Editor(std::u16string text, SkSize size)
    ...
    // static auto fontChain = sk_make_sp<TrivialFontChain>("Roboto", 40, SkFontStyle::Normal());
    // printf("fontChain %p\n", fontChain.get());
    // sk_sp<SkTypeface> typeface = fontChain->getTypeface();
    // printf("fontChain.getTypeface() %p\n", typeface.get());
