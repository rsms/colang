...
static Node* resolve_id(Parser* p, Node* id) {
  ...
  } else if (!NodeKindIsExpr(id->ref.target->kind)) {
    // The target is a constant or type; short-circuit and return that instead of the id.
    // Example:
...
static Node* pArrayType(Parser* p, PFlag fl) {
  ...
  if (fl&PFlagRValue) {
    panic("TODO PListInfix rvalue => param?");
...
static const Parselet parselets[TMax] = {
  ...
  [TLBrack] = {NULL, PListInfix, PREC_LOWEST},
