...
// Always sets errmsg
static error link_main(LinkFun linkf, llvm::ArrayRef<const char*> args) {
  if (_lld_is_corrupt)
    return err_mfault;
  ...
  error err = 0;
  {
    ...
    if (!crc.RunSafely([&]() {
      if (linkf(args, llvm::outs(), errout, exitEarly, false) == false)
        err = err_invalid; // TODO: better error code
    }))
    {
...
error llvm_link(const CoLLVMLink* optionsptr) {
  ...
  LinkFun linkfn;
  error err = build_args(options, triple, &linkfn, args, tmpstrings);
  if (err)
    return err;
