...
static Node* nullable ASTVisitorNoop(ASTVisitor* v, Node* n, Node* pn, const char* pf) {
  if (!is_LitExpr(n)) ASTVisitChildren(v, n);
  return n;
}

void ASTVisitorInit(ASTVisitor* v, const ASTVisitorFuns* f, void* nullable ctx) {
  v->ctx = ctx;
  pmap_init(&v->seenmap, mem_ctx(), 64, MAPLF_1);
