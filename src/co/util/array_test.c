...
R_TEST(array_heap) {
  ...
  Array a = Array_INIT;
  ArrayPush(&a, (void*)1, mem); // visits ArrayGrow's "onheap" branch
  ArrayPush(&a, (void*)2, mem);
  ArrayPush(&a, (void*)3, mem);
...
R_TEST(array_stack_to_heap) {
  ...
  ArrayPush(&a, (void*)1, mem);
  asserteq(a.onstack, true);
  ArrayPush(&a, (void*)2, mem);
  asserteq(a.onstack, true);
  ArrayPush(&a, (void*)3, mem);  // visits ArrayGrow's "move stack to heap" branch
...
R_TEST(array_copy) {
  ...
  auto items = (void**)memalloc(mem, nitems * sizeof(void*));
  auto len1 = a2.len;
  ArrayCopy(&a2, len1, items, nitems, mem);
