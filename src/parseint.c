...
error _parseint_u64(const char* src, usize z, int base, u64* result, u64 cutoff) {
...
error parseint_i64(const char* src, usize z, int base, i64* result) {
  while (z > 0 && *src == '-') {
    src++;
    z--;
  }
  i64 r;
  error err = parseint_u64(src, z, base, (u64*)&r);
  if (err)
    return err;
  if (*src == '-')
    r = -r;
...
#define check_uresult(fun, input, base, err, expect, result) { \
...
#define check_sresult(fun, input, base, err, expect, result) { \
  check_err(fun, input, base, err); \
  assertf(result == expect, \
    "\ninput:   \"%s\" base=%d\nexpected %20lld\ngot      %20lld\n", \
    input, base, expect, result); \
...
static void test_u64(const char* cstr, int base, u64 expectnum) {
  u64 result = 0;
  error err = parseint_u64(cstr, strlen(cstr), base, &result);
  check_uresult(parseint_u64, cstr, base, err, expectnum, result);
  if (base == 10) {
    // test the generic implementation for base-10
    error err = _parseint_u64(cstr, strlen(cstr), base, &result, 0xFFFFFFFFFFFFFFFF);
    check_uresult(_parseint_u64, cstr, base, err, expectnum, result);
  }
}

static void test_i64(const char* cstr, int base, i64 expectnum) {
  i64 result = 0;
  error err = parseint_i64(cstr, strlen(cstr), base, &result);
  check_sresult(parseint_i64, cstr, base, err, expectnum, result);
}

static void test_u32(const char* cstr, int base, u32 expectnum) {
  u32 result = 0;
  error err = parseint_u32(cstr, strlen(cstr), base, &result);
  check_uresult(parseint_u32, cstr, base, err, (u64)expectnum, (u64)result);
  if (base == 10) {
    // test the generic implementation for base-10
    u64 result = 0;
    error err = _parseint_u64(cstr, strlen(cstr), base, &result, 0xFFFFFFFF);
    check_uresult(_parseint_u64_as_u32, cstr, base, err, (u64)expectnum, result);
  }
}
