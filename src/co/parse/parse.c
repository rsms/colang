...
static void syntaxerrp(Parser* p, SrcPos pos, const char* format, ...) {
  ...
  if (p->ctx->errh)
    p->ctx->errh(p->s.src, pos, msg, p->ctx->userdata);
...
static Scope* pushScope(Parser* p) {
  auto s = ScopeNew(p->scope, p->ctx->mem);
  p->scope = s;
...
static Node* tupleTrailingComma(Parser* p, int precedence, PFlag fl, Tok stoptok) {
  ...
  do {
    NodeListAppend(p->ctx->mem, &tuple->array.a, expr(p, precedence, fl));
...
static Node* PBlock(Parser* p, PFlag fl) {
  ...
  while (p->s.tok != TNone && p->s.tok != TRBrace) {
    NodeListAppend(p->ctx->mem, &n->array.a, exprOrTuple(p, PREC_LOWEST, fl));
    if (!got(p, TSemi)) {
      break;
    }
  }
  ...
  while (p->s.tok != TRParen && p->s.tok != TNone) {
    ...
    if (p->s.tok == TId) {
      ...
      if (p->s.tok != TRParen && p->s.tok != TComma && p->s.tok != TSemi) {
        ...
        NodeListAppend(p->ctx->mem, &typeq, field);
      }
    } else {
      // definitely just type, e.g. "fun(int)int"
      field->type = expr(p, PREC_LOWEST, fl);
    }
    NodeListAppend(p->ctx->mem, &n->array.a, field);
    if (!got(p, TComma)) {
      if (p->s.tok != TRParen) {
...
//PARSELET_MAP_BEGIN
// automatically generated by /Users/rsms/src/wlang/misc/gen_parselet_map.py; do not edit
...
static Node* exprOrTuple(Parser* p, int precedence, PFlag fl) {
  ...
  if (got(p, TComma)) {
    ...
    NodeListAppend(p->ctx->mem, &g->array.a, left);
    if (fl & PFlagRValue) {
      do {
        NodeListAppend(p->ctx->mem, &g->array.a, expr(p, precedence, fl));
      } while (got(p, TComma));
    } else {
      do {
        NodeListAppend(p->ctx->mem, &g->array.a, prefixExpr(p, fl));
      } while (got(p, TComma));
    }
    left = g;
  }
...
Node* Parse(Parser* p, BuildCtx* ctx, Source* src, ParseFlags fl, Scope* pkgscope) {
  ...
  while (p->s.tok != TNone) {
    ...
    NodeListAppend(p->ctx->mem, &file->array.a, n);
    ...
    // auto s = NodeRepr(n, str_new(32));
    // s = str_appendc(s, '\n');
    // fwrite(s, str_len(s), 1, stdout);
    // str_free(s);
...
// static Node* PList(Parser* p, const Parselet* e, PFlag fl, Node* left) {
//   Node* n = left;
//   if (left->kind != NList) {
//     n = mknode(p, NList);
//     nlistPush(n, left);
...
static Node* pVarMulti(Parser* p, NodeKind nkind, Node* ident0) {
  auto names = mknode(p, NTuple);
  names->pos = ident0->pos;

  NodeListAppend(p->ctx->mem, &names->array.a, ident0);
  do {
    NodeListAppend(p->ctx->mem, &names->array.a, pident(p));
  } while (got(p, TComma));
...
// Tuple = Expr ("," Expr)+
static Node* ptuple(Parser* p, int precedence) {
  auto tuple = mknode(p, NTuple);
  do {
    NodeListAppend(p->ctx->mem, &tuple->array.a, expr(p, precedence));
  } while (got(p, TComma));
  return tuple;
}
